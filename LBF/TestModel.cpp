...
double TestModel (vector<string> testDataName, string _folderPath){
    ...
    for(int i=0;i<testDataName.size();i++){
        ...
        LoadOpencvBbxData(folderPath + dataPath + testDataName[i],path, test_images, test_ground_truth_shapes, test_bounding_boxs);
    }
//    LoadCofwTestData(test_images, test_ground_truth_shapes, test_bounding_boxs);
    
    
    LBFRegressor regressor;
    regressor.Load(_folderPath+modelPath+"LBF.model");
